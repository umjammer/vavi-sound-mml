...
public class CustomMusicDataModifier extends MusicDataModifier {
    ...
    protected ChangeEventSet CreateInstrumentSet(Instrument i) {
        return new ChangeEventSet(i, new CustomInstrument((i.getValue() + 1), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreatePanSet(Pan p) {
        return new ChangeEventSet(p, new CustomPan(p.getValue(), MMLCommandRelation.Clear));
    }
    ...
    protected TempoSet CreateTempoSet(Tempo t) {
        return new TempoSet(t, new CustomTempo(t.getValue(), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreateVolumeSet(Volume v) {
        return new ChangeEventSet(v, new CustomVolume(v.getValue(), MMLCommandRelation.Clear));
    }
}
