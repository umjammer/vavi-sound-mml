...
public class Mml2vgmMusicDataModifier extends MusicDataModifier {
    ...
    protected ChangeEventSet CreateInstrumentSet(Instrument i) {
        return new ChangeEventSet(i, new Mml2vgmInstrument((i.getValue() + 1), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreatePanSet(Pan p) {
        return new ChangeEventSet(p, new Mml2vgmPan(p.getValue(), MMLCommandRelation.Clear));
    }
    ...
    protected TempoSet CreateTempoSet(Tempo t) {
        return new TempoSet(t, new Mml2vgmTempo(t.getValue(), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreateVolumeSet(Volume v) {
        return new ChangeEventSet(v, new Mml2vgmVolume(v.getValue(), MMLCommandRelation.Clear));
    }
}
