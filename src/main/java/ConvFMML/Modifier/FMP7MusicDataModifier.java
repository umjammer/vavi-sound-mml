...
public class FMP7MusicDataModifier extends MusicDataModifier {
    ...
    protected ChangeEventSet CreateInstrumentSet(Instrument i) {
        return new ChangeEventSet(i, new FMP7Instrument((i.getValue() + 1), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreatePanSet(Pan p) {
        return new ChangeEventSet(p, new FMP7Pan(p.getValue(), MMLCommandRelation.Clear));
    }
    ...
    protected TempoSet CreateTempoSet(Tempo t) {
        return new TempoSet(t, new FMP7Tempo(t.getValue(), MMLCommandRelation.Clear));
    }

    @Override
    protected ChangeEventSet CreateVolumeSet(Volume v) {
        return new ChangeEventSet(v, new FMP7Volume(v.getValue(), MMLCommandRelation.Clear));
    }
}
