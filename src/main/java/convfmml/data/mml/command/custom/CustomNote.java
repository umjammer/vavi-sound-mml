...
public class CustomNote extends Note {
    ...
    public CustomNote(int octave, String name, List<Integer> length, EnumSet<MMLCommandRelation> relation) {
    ...
    protected String generateString(Settings settings, SoundModule module) {
        ...
        for (int i = 1; i < length.size(); i++) {
            if (len * 2 == length.get(i) && nrSettings.isDotEnable() &&
                ...
                if (nrSettings.getTieStyle() == 0) { // Tie and Name
                    str.append(nrSettings.getTieCommandCustom()).append(name).append(length.get(i));
                } else { // Tie only
                    str.append(nrSettings.getTieCommandCustom()).append(length.get(i));
                }
                dotlen = 0;
            }
            len = length.get(i);
        ...
        if (getCommandRelation().contains(MMLCommandRelation.TieAfter)) {
            str.append(nrSettings.getTieCommandCustom());
        }
